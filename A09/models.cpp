...
*/
int stackCount=10,sectorCount=10;
    int cradius=2;
    ...
    int contatore=0;
    int vIndex=0;
    M3_vertices.resize(((sectorCount+1)*(stackCount+1))*3);
    for(int i = 0; i <= stackCount; ++i)
    {
        stackAngle = (M_PI / 2) - (i * stackStep);        // starting from pi/2 to -pi/2
        xy = cradius * cosf(stackAngle);             // r * cos(u)
        z = cradius * sinf(stackAngle);              // r * sin(u)
        printf("-------\n");
        ...
        for(int j = 0; j <= sectorCount; ++j) {
            ...
            M3_vertices[vIndex]=x;
            M3_vertices[vIndex+1]=y;
            M3_vertices[vIndex+2]=z;
            vIndex=vIndex+3;
    ...
    for(int i = 0; i < stackCount; ++i)
    {
        ...
        for(int j = 0; j <sectorCount; ++j, ++k1, ++k2) {
            ...
            if (i != 0 ) {
                M3_indices[index]=(k1+1) ;
                M3_indices[index+1]=(k2) ;
                M3_indices[index+2]=(k1);
                index=index+3;
            }
            else{
                printf("indices %d with k1=%d and k2=%d \n " , index,k1,k2);
            }
            if (i != (stackCount-1)){
                M3_indices[index]=(k1+1);
                M3_indices[index+1]=(k2);
                M3_indices[index+2]=(k2+1);
                index=index+3;
            }
            else{
                printf("indices %d with k1=%d and k2=%d \n " , index,k1,k2);
